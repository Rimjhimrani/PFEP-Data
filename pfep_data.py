...
def manual_review_step(df, internal_key, step_name):
    ...
    uploaded_file = st.file_uploader(f"Upload the modified '{step_name}' file (optional)", type=['csv'])
    if uploaded_file:
        try:
...
def main():
    ...
    with st.expander("STEP 1: Upload Data Files & Set Parameters", expanded=True):
        ...
        with col1:
            ...
            pbom_files = st.file_uploader("Upload PBOM files", accept_multiple_files=True, type=['csv', 'xlsx'])
        with col2:
            st.subheader("📁 UPLOAD MBOM FILES")
            mbom_files = st.file_uploader("Upload MBOM files", accept_multiple_files=True, type=['csv', 'xlsx'])
        with col3:
            st.subheader("🚚 UPLOAD VENDOR MASTER FILE")
            vendor_file = st.file_uploader("Upload Vendor Master file", type=['csv', 'xlsx'])
    ...
    if st.session_state.processor:
        ...
        # --- Step 2 to 4: Classifications ---
        st.header("--- PROCESSING STEPS ---")
        
        # (Step 1/6) Family Classification
        with st.expander("(1/6) Family Classification"):
        ...
        # (Step 2/6) Size Classification
        with st.expander("(2/6) Size Classification"):
        ...
        # (Step 3/6) Part Classification
        with st.expander("(3/6) Part Classification (Percentage-Based)"):
        ...
        # (Step 4/6) Location-Based Norms
        with st.expander("(4/6) Distance & Inventory Norms"):
        ...
        # (Step 5/6) Warehouse Location Assignment
        with st.expander("(5/6) Warehouse Location Assignment"):
